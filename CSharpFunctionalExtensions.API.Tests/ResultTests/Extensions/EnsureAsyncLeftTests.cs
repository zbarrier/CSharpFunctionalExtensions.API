...
namespace CSharpFunctionalExtensions.API.Tests.ResultTests.Extensions {
    public class EnsureAsyncLeftTests {
        ...
        public async Task Ensure_with_errorPredicate_does_not_throw_when_given_result_failure() {
            var result = Task.FromResult(Result.Failure<string>(NewError("initial error message")));
            Func<Task> ensure = () => result.Ensure(
                x => x != "",
                x => NewError("new error message: string should not be empty"));
        ...
        public async Task Ensure_with_errorPredicate_initial_result_has_failure_state() {
            var tResult = Task.FromResult(Result.Failure<string>(NewError("initial error message")));

            var result = await tResult.Ensure(
                x => x != "",
                x => NewError("new error message: string should not be empty"));

            result.IsSuccess.Should().BeFalse("Input Result.Failure should be returned");
            result.Error.Message.Should().Be("initial error message");
        }
        ...
        public async Task Ensure_with_errorPredicate_using_errorPredicate() {
            ...
            result.Error.Message.Should().Be("new error message: string should not be empty");
        }
